...
export default function PurchaseStatus(props: {purchase: Purchase}) {
    ...
    useEffect(() => {
        const setStatus = async () => {
            setPayment(await fetchPayment(props.purchase.id_transaccion).then(resp => {return resp.estado}))
            setShipment(await fetchShipment(props.purchase.id_envio).then(resp => {return resp.estado}))
        }
        setStatus()
    }, [])
